...
for (int i = 1; arg != NULL && i < argn; i++) arg = arg->next;
...
previous lines have been obeyed. There is nothing to do, but check for errors.
...
do_endarg(uschar *p)
{
if (from_type[from_type_ptr] != FROM_MACRO) error(15, US".endarg");
else if (*p != 0) error(19, ".endarg", p, 8, spaces, Ustrlen(p), circumflexes);
}
...
the previous lines have been obeyed. There is nothing to do, but check for
errors.
...
{
if (from_type[from_type_ptr] != FROM_MACRO) error(15, US".endinliteral");
else if (*p != 0) error(19, ".endinliteral", p, 8, spaces, Ustrlen(p), circumflexes);
}
...
f->length1 = length;
f->length2 = 0;   /* In case of error call to free */
f->flag1 = misc_copystring(p, length);
f->rep1 = NULL;   /* In case of error call to free */

/* Now look backwards from the end of the line and find the last quoted string
that is there. */

q = pp + Ustrlen(pp);
if (*(--q) != '\"' && *q != '\'') { error(11, thisdir); flag_free(f); return; }
...
if (q <= pp) { error(11, thisdir); flag_free(f); return; }
...
/* Otherwise, we are dealing with a pair of flags. */
  ...
  length = pp - p;
  if (length == 0) { error(10); flag_free(f); return; }
  f->length2 = length;
  f->flag2 = misc_copystring(p, length);
  while (isspace(*pp)) pp++;
  if (*pp != '\"' && *pp != '\'') { error(11, thisdir); flag_free(f); return; }
  f->rep1 = misc_readstring(pp, &length, NULL, 0);
  if (pp + length >= q) { error(11, thisdir); flag_free(f); return; }
  }
...
if (from_type_ptr >= FROM_TYPE_STACKSIZE - 1) error(32);  /* Hard error */
...
int len = Ustrlen(p);
if (len > INBUFFSIZE - 1) len = INBUFFSIZE - 1;
